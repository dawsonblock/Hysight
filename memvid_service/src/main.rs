...
impl SidecarVecIndex {
    ...
    fn search(&self, query: &[f32], limit: usize) -> Vec<(String, f32)> {
        ...
        let mut dest =
            vec![space::Neighbor::<u32> { index: !0, distance: 0 }; ef];
...
impl PersistentMemoryStore {
    fn new(data_dir: PathBuf) -> Result<Self, Box<dyn std::error::Error + Send + Sync>> {
        ...
        for entry in entries {
            ...
            if deleted_ids.contains(&memory_id) { continue; }
            if meta.get("hca_expired").map(|s| s == "true").unwrap_or(false) { continue; }
            ...
            if let Some(f2) = frame2 {
                ...
                records.insert(
                    ...
                    MemoryRecord {
                        ...
                        confidence:   meta2.get("hca_confidence").and_then(|s| s.parse().ok()).unwrap_or(0.5),
                        scope:        meta2.get("hca_scope").cloned().unwrap_or_else(|| "private".into()),
        ...
        Ok(Self { memvid, records, vec_index, deleted_ids, deleted_ids_path })
    }
    ...
    ) -> Result<String, memvid_core::MemvidError> {
    ...
    fn retrieve_bm25(&mut self, query: &RetrievalQuery) -> Vec<RetrievalHit> {
        ...
        for hit in response.hits {
            ...
            if !self.passes_filter(record, query) { continue; }
    ...
    fn retrieve_semantic(&self, query: &RetrievalQuery) -> Vec<RetrievalHit> {
        let Some(ref emb) = query.embedding else {
            tracing::warn!("semantic mode requested but no embedding in query — fallback to no results");
            return vec![];
        };
    ...
    fn retrieve_hybrid(&mut self, query: &RetrievalQuery) -> Vec<RetrievalHit> {
        ...
        merged
            ...
            .filter_map(|(mid, score)| {
                self.records.get(&mid).map(|rec| self.make_hit(rec, score))
            })
            .collect()
    }

    fn passes_filter(&self, record: &MemoryRecord, query: &RetrievalQuery) -> bool {
        if record.expired && !query.include_expired { return false; }
        if let Some(ref ml) = query.memory_layer {
            if ml != "trace" { return false; }
        }
        if let Some(ref s) = query.scope {
            if s != &record.scope { return false; }
        }
        if let Some(ref rid) = query.run_id {
            if record.run_id.as_deref() != Some(rid.as_str()) { return false; }
        }
        true
    }
    ...
    fn maintain(&mut self) -> MaintenanceReport {
        ...
        for (id, rec) in &self.records {
            ...
            } else if matches!(rec.memory_type.as_str(), "fact" | "episode" | "preference" | "goalstate" | "procedure") {
                durable += 1;
            }
        }
        for id in &to_expire {
    ...
    fn delete(&mut self, memory_id: &str) -> bool {
        if self.records.remove(memory_id).is_some() {
            ...
            if let Ok(mut f) = OpenOptions::new()
                .create(true).append(true).open(&self.deleted_ids_path)
            {
...
async fn ingest_handler(
    ...
    let user_store = {
        let mut shared = state.lock().map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
        get_or_create_user_store(&mut shared, &user_id)?
    };
    let memory_id = user_store
        .lock().map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?
        .ingest(candidate)
        .map_err(|e| { tracing::error!("Ingest: {e}"); StatusCode::INTERNAL_SERVER_ERROR })?;
...
async fn list_handler(
    ...
    let guard = user_store.lock().map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
...
async fn delete_handler(
    ...
    let user_id = q.get("user_id").cloned().unwrap_or_else(|| "default".into());
    let user_store = {
        let mut shared = state.lock().map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
        get_or_create_user_store(&mut shared, &user_id)?
    };
...
async fn main() {
    ...
    let default_store = match PersistentMemoryStore::new(default_dir) {
        Ok(s)  => s,
        Err(e) => { tracing::error!("Cannot create default store: {e}"); std::process::exit(1); }
    };
